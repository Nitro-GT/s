...
Theme.SoundId = getcustomasset("Collection/LastStarMusic/apex.mp3")
...
function formswitch(textcolor,textstrokecolor,theme,primary,secondary,walkspeed,jumppower,attack,formname,formfont,wingstyle,formidle,themevolume,fancyexplosionbool,altindexnumber,corebool,scriptname,dynamic,themename,extrawingsbool,marbleimage)
	...
	local oldtime = Theme.TimePosition
		...
		Theme:Play()
		if Theme.TimeLength > oldtime then
		Theme.TimePosition = oldtime
		end
	end
	...
	if key == Enum.KeyCode.B then
		...
		if key == Enum.KeyCode.KeypadPlus then
			if Theme.Playing then
			    juicy=game:GetService("RunService").Heartbeat:Connect(function()
				    Theme.Playing = false
				end)
			else
			    juicy:Disconnect()
...
script.Parent.Settings.EffectLimit.Value = tonumber(250)
